...
def generate_spatial(mp_args):
    ...
    #print("Generating spatial",config)
    ...
    for w_idx in idx_rand:
        ...
        labels[w_idx] = label
    return imgs, labels, config_list, i
...
def generate_single(mp_args):
    ...
    def update_config(idx):
        if colors.count(colors[idx])==1 and shapes.count(shapes[idx])==1:
            if np.random.randint(0,2) == 0:
                config = SingleConfig(colors[idx],None)
            else:
                config = SingleConfig(None,shapes[idx])
        elif colors.count(colors[idx])==1:
            config = SingleConfig(colors[idx],None)
        elif shapes.count(shapes[idx])==1:
            config = SingleConfig(None,shapes[idx])
        else:
            config = SingleConfig(colors[idx],shapes[idx])
        return config
        
    return imgs, labels, [update_config(i) for i in range(len(config_list))], i
...
def generate(n,
    ...
    for imgs, labels, config_list, i in gen_iter:
        all_imgs[i, ] = imgs
    ...
    langs = np.array([" # ".join([fmt_config(c) for c in c_list]) for c_list in configs], dtype=np.unicode)
    #print(langs)
...
if __name__ == '__main__':
    ...
    data_dir = './data/all_langs/all_langs_reference-1000-'
