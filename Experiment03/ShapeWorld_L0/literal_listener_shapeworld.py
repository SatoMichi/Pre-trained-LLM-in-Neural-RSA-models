...
class SimpleBaseLine_ShapeWorld_L0(nn.Module):
    def __init__(self,vocab_size, emb_dim=1024,device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) -> None:
...
class ShapeWorld_BERT_Sent_L0(nn.Module):
    def __init__(self,emb_dim=768,hidden_dim=1024,device=torch.device("cuda" if torch.cuda.is_available() else "cpu")) -> None:
        super(ShapeWorld_BERT_Sent_L0,self).__init__()
        self.embedding_dim = emb_dim
        self.hidden_dim = hidden_dim
        self.liner = nn.Linear(emb_dim,hidden_dim)
        self.cnn_color_encoder = CNN_encoder(6)
        self.cnn_color_encoder.load_state_dict(torch.load("model_params/shapeworld_original-cnn_color_model.pth"))
        self.cnn_shape_encoder = CNN_encoder(4)
        self.cnn_shape_encoder.load_state_dict(torch.load("model_params/shapeworld_original-cnn_shape_model.pth"))
        self.to_hidden = nn.Linear(10,self.hidden_dim)
